...
export default function AppointmentCard() {
  ...
  const renderCalendarDays = () => {
    ...
    for (let day = 1; day <= remainingCells; day++) {
      days.push(
        <div
          key={`next-${day}`}
          className="w-6 h-6 flex items-center justify-center text-green-900 opacity-50 text-md"
        >
          {day}
        </div>
      );
  ...
  return (
    <div className="w-[330px] mx-auto bg-white rounded-xl border-1 border-green-100  p-2 ml-0.">
      {/* Header */}
      <h2 className="text-lg font-medium text-green-950 mb-4">
        Next Appointment
      </h2>
      ...
      <div className="grid grid-cols-7 gap-0.5 mb-2">
        {dayNames.map((day) => (
          <div
            key={day}
            className="text-[10px] font-semibold text-green-900 text-center"
          >
            {day}
          </div>
        ))}
